...
\begin{abstract}
We present \textit{Ashta Padi}, a profession-based framework for Sanskrit scripture learning integrating personalized vocabulary acquisition, grammar grounding, and structured certification progression. Unlike existing tools focused on textual analysis, Ashta Padi emphasizes learner onboarding and pedagogical progression. This demo implements Steps~1--3 using \textit{Yoga S\=utras}, allowing users to select professions, interact with relevant vocabulary, and study sutras through a dual-panel interface with contextual grammar, including word-level grammatical explanations and sutra-wise progress tracking.
\end{abstract}
...
Ashta Padi defines an eight-stage progression from introductory exposure to advanced scholarship and certification. While pedagogical in scope, the framework relies on computational mappings between professions, vocabulary sets, grammatical concepts, and learning checkpoints. The current demo focuses on Steps~1--3.

\begin{figure}[h]
\centering
\includegraphics[width=0.25\linewidth]{complete_ashta_padi_model.png}
\caption{Overview of the eight-step Ashta Padi learning framework.}
\label{fig:model}
\end{figure}

\section{System Architecture and Demo Implementation}
The Ashta Padi system architecture comprises three layers: (1) a \textbf{Content Layer} containing curated Sanskrit texts, vocabulary, and grammatical references; (2) a \textbf{Pedagogy Layer} implementing rule-based mappings between professions and learning sequences; and (3) an \textbf{Interface Layer} providing an interactive web-based learning environment.

\subsection{Demo Scope}
The live demo implements Steps~1--3 using the \textit{Yoga S\=utras, Sam\=adhi P\=ada}. Key features include:

\begin{itemize}
\item Profession-aware vocabulary presentation and progress tracking
\item Dual-panel interface with Devanagari, transliteration, translation, and contextual grammar references
\item \textbf{Mentor Selection (Step 3):} Workflow to choose academic mentors (e.g., Profs. Srinivas Varkhedi, Gauri Mahulikar, Amba Kulkarni, Anuradha Chaudhary, Malhar Kulkarni) for live weekly/monthly Q\&A
\item \textbf{Ecosystem Integration:} Conceptual support for third-party Sanskrit libraries/dictionaries, mimicking the ``Grammarly-in-Gmail'' model to assist term comprehension
\end{itemize}

\begin{figure}[h]
\centering
\includegraphics[width=0.75\linewidth]{demo_screenshot.png}
\caption{Dual-panel learning interface showing sutra content (left) with Devanagari, transliteration, translation, and commentary, alongside contextual grammar references (right).}
\label{fig:demo}
\end{figure}
